...
const media_next_prev = media_meta => {
	...
	prev_btn.onclick = event => {
		...
		let prev_ele = document.querySelector('[data-id="' + media_meta.dataset.id + '"]').parentElement.previousElementSibling;
	...
	next_btn.onclick = event => {
		...
		let next_ele = document.querySelector('[data-id="' + media_meta.dataset.id + '"]').parentElement.nextElementSibling;
...
const media_like_dislike = media_meta => {
	...
	like_btn.onclick = event => {
		...
		fetch('like-dislike.php?id=' + media_meta.dataset.id + '&type=like').then(res => res.text()).then(data => {
			// Increment the thumbs up value
			if (data.includes('success')) {
				media_popup.querySelector('.thumbs-up-count').innerHTML = parseInt(media_popup.querySelector('.thumbs-up-count').innerHTML) + 1;
	...
	dislike_btn.onclick = event => {
		event.preventDefault();
		// Use AJAX to update the value in the database
		fetch('like-dislike.php?id=' + media_meta.dataset.id + '&type=dislike').then(res => res.text()).then(data => {
			// Increment the thumbs down value
			if (data.includes('success')) {
				media_popup.querySelector('.thumbs-down-count').innerHTML = parseInt(media_popup.querySelector('.thumbs-down-count').innerHTML) + 1;
...
const media_preview = file => {
    ...
    reader.onload = () => {
		...
		if (file.type.toLowerCase().includes('image')) {
			document.querySelector('#preview').innerHTML = `<img src="${reader.result}" alt="preview" style="max-height:300px;max-width:100%;">`;
		}
		if (file.type.toLowerCase().includes('audio')) {
			document.querySelector('#preview').innerHTML = `<audio src="${reader.result}" controls style="width:100%"></audio>`;
		}
		if (file.type.toLowerCase().includes('video')) {
			document.querySelector('#preview').innerHTML = `<video src="${reader.result}" style="max-height:300px;width:100%;" controls></video>`;
		}
    };
...
if (media_popup) {
				...
				img.onload = () => {
					...
					media_popup.innerHTML = `
						...
						<i class="fa-solid fa-xmark"></i>
							...
							<h3>${media_meta.dataset.title}</h3><small style='margin-top:0.25em'>made in : ${media_meta.dataset.year} - nr. ${media_meta.dataset.fnr} </small>
							<p>${media_meta.alt}</p>
	...
	media_popup.onclick = e => {
		if (e.target.className == 'media-popup' || e.target.className == 'fa-solid fa-xmark' || e.target.className == 'rj-close-popup') {
...
if (upload_form) {
	...
	upload_form.onsubmit = event => {
		...
		request.upload.addEventListener('progress', event => {
			...
			upload_form.querySelector('#submit_btn').value = 'Uploading... ' + '(' + ((event.loaded/event.total)*100).toFixed(2) + '%)';
		...
		request.onreadystatechange = () => {
		    if (request.readyState == 4 && request.status == 200) {
				...
				if (request.responseText.includes('Complete')) {
					...
					document.querySelector('.upload .msg').innerHTML = request.responseText;
				}
		    }
		};
		// Send the request
	...
	upload_media.onchange = () => {
		if (upload_media.files[0].type.toLowerCase().includes('audio') || upload_media.files[0].type.toLowerCase().includes('video')) {
			document.querySelectorAll('.thumbnail').forEach(el => el.style.display = 'flex');
		} else {
			document.querySelectorAll('.thumbnail').forEach(el => el.style.display = 'none');
		}
	...
	upload_drop_zone.ondrop = event => {
		...
		if (event.dataTransfer.items && event.dataTransfer.items[0].kind === 'file') {
			...
			if (file.type.toLowerCase().includes('audio') || file.type.toLowerCase().includes('video') || file.type.toLowerCase().includes('image')) {
				upload_file = file;
